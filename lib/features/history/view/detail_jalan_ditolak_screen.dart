...
class DetailJalanDitolakScreen extends StatefulWidget {
  final int index;
  final Datum data;
  const DetailJalanDitolakScreen(
      {super.key, required this.index, required this.data});

  @override
  State<DetailJalanDitolakScreen> createState() =>
      _DetailJalanDitolakScreenState();
}

class _DetailJalanDitolakScreenState extends State<DetailJalanDitolakScreen> {
  @override
      ...
      body: SingleChildScrollView(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              item("Nama Segmen",
                  widget.data.segmens![widget.index].segmen!.name ?? ""),
              const SizedBox(
                height: 16,
              ),
              item("Jenis Kerusakan",
                  widget.data.segmens![widget.index].userType ?? ""),
              const SizedBox(
                height: 16,
              ),
              item("Tingkat Kerusakan",
                  widget.data.segmens![widget.index].userLevel ?? ""),
              const SizedBox(
                height: 16,
              ),
              ...
              SizedBox(
                ...
                child: ListView.separated(
                  shrinkWrap: true,
                  itemCount: widget.data.segmens![widget.index].photos!.length,
                    ...
                    return InkWell(
                      onTap: () {
                        _showFullScreenImage(
                            context,
                            widget.data.segmens![widget.index].photos![index]
                                    .absPath ??
                                "");
                      },
                      child: ClipRRect(
              ...
              item("Panjang Segmen",
                  "${widget.data.segmens![widget.index].segmen!.length == null ? 0 : widget.data.segmens![widget.index].segmen!.length!.toStringAsFixed(2)} m"),
              const SizedBox(
                height: 16,
              ),
              Consumer<DetailHistoryProvider>(
                  builder: (context, detailHistoryProvider, _) {
                return Container(
                  ...
                  child: FlutterMap(
                    ...
                    options: MapOptions(
                      center: detailHistoryProvider.listPolyline[0].points[0],
                      zoom: 19,
                      maxZoom: 19,
                    ...
                    children: [
                      ...
                      PolylineLayer(
                        polylines: detailHistoryProvider.listPolyline,
                      ),
                    ],
                  ),
                );
